...
while True:
    ...
    if start == 0:
        ...
        enemy_number, enemies_speed, enemies_bullet_speed, enemies_firing_rate, \
            speed, bullet_speed, firing_rate, map_ = UI.interface(screen)
    ...
    # 每隔 1.5秒生成一个敌方坦克
    t2 = time.time()
    if t2 - t1 > 1.5 and enemy_num < enemy_number and len(enemies) < 5 and tank_0.stop2 != 1:
    ...
    walls.update()
    enemies.update()
    game_over, destroyed_enemies = otherCollisions(bullets, bullets_1, enemies, myTank, game_over, destroyed_enemies)
    ...
    if destroyed_enemies == enemy_number:
        tank_0.game_over = 1
        x, y = pygame.mouse.get_pos()
