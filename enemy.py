...
class Enemy(Sprite):
    def __init__(self, screen, bullets_1, move_direction,
                 tank_0, walls, speed, bullet_speed, firing_rate, tank0, bpx, bpy):
        super().__init__()
        self.tank_0 = tank0
        self.screen = screen
        self.walls = walls
    ...
    def update(self):
        ...
        if self.mode == 1 and self.stop == 0:
            t4 = time.time()
            t8 = time.time()
            if t4 - self.t3 >= 1.3:
                ...
                if t8 - self.t7 > 0.002:
                    self.nowgd, self.x3, self.y3 = change_direction(self.lastgd, self.nowgd, self.nextgd,
                                                                    self.rect.centerx, self.rect.centery,
                                                                    self.move_direction, self.speed)
                    self.t7 = time.time()

        if self.mode == 2 and self.stop == 0:
            t14 = time.time()
            t16 = time.time()
            if t14 - self.t13 >= 0.1:
